...
function createTimingFailure({ message, stdout, stderr, cause, exitCode = null }) {
  ...
  error.name = 'FasterWhisperDialogueTimingError';
  error.stdout = stdout;
  error.stderr = stderr;
...
async function deriveFasterWhisperDialogueTiming({
  assetPath,
  spawnImpl = spawn,
  ...
  if (!assetPath) {
    throw new Error('deriveFasterWhisperDialogueTiming requires assetPath.');
  }
  ...
  return await new Promise((resolve, reject) => {
    ...
    child.on('close', (code) => {
      ...
      const dialogueSegments = Array.isArray(payload?.dialogue_segments) ? payload.dialogue_segments : null;
      const timedSegmentCount = dialogueSegments
        ? dialogueSegments.filter((segment) => Number.isFinite(segment?.start) && Number.isFinite(segment?.end)).length
        : 0;

      if (!dialogueSegments || timedSegmentCount === 0) {
        reject(createTimingFailure({
          message: 'faster-whisper bridge did not return any timed dialogue segments.',
          stdout,
          stderr
        }));
        return;
