...
### Task 9: Compare the fresh OpenRouter MiMo dialogue output against cod-test benchmark truth and prior dialogue benchmark results

**Bead ID:** `ee-27za`
**SubAgent:** `research`
**Prompt:** `In /home/derrick/.openclaw/workspace/projects/peanut-gallery/emotion-engine, compare the fresh dialogue output from output/cod-test-mimo-openrouter-compare/phase1-gather-context/dialogue-data.json against the cod-test dialogue benchmark truth and the repo's prior dialogue benchmark results. Use the existing benchmark manifest/truth/comparator machinery where possible rather than ad-hoc eyeballing. Produce a concise evidence-backed summary of how the fresh OpenRouter MiMo dialogue output compares to the old benchmarked baseline, where it matches truth better or worse, and what the most important remaining mismatch buckets are. Update the active plan with what actually happened, claim bead ee-27za on start with bd update ee-27za --status in_progress --json, and close it on completion with bd close ee-27za --reason "Compared fresh OpenRouter MiMo dialogue output to cod-test benchmark truth and prior results" --json.`
...
**Files Created/Deleted/Modified:**
- `.plans/2026-04-01-openrouter-mimo-dialogue-json-audit.md`
- comparison artifacts/notes to be determined

**Status:** ✅ Complete

**Results:** Ran the existing JSON structured dialogue comparator (`server/lib/benchmark-runner.cjs`, `dialogue-default` profile) against the fresh artifact at `output/cod-test-mimo-openrouter-compare/phase1-gather-context/dialogue-data.json` by scaffolding a temporary manifest that reuses the cod-test truth/comparator contract but writes reports to `tmp/openrouter-mimo-dialogue-benchmark/`. Durable evidence: `tmp/openrouter-mimo-dialogue-benchmark/reports/benchmark-summary.json`, `tmp/openrouter-mimo-dialogue-benchmark/reports/artifact-results/dialogueData.json`, `tmp/openrouter-mimo-dialogue-benchmark/comparison-vs-dialogue-baseline.json`, `tmp/openrouter-mimo-dialogue-benchmark/common-path-delta.json`, and the human summary `docs/research/2026-04-02-openrouter-mimo-dialogue-benchmark-comparison.md`. Headline: the fresh OpenRouter MiMo dialogue output is still benchmark-`error`, but it modestly outperforms the repo’s prior dialogue baseline on comparable truth paths. Raw report delta vs baseline: accuracy `37.25%` vs `35.65%` (`+1.59` pts), passed fields `92` vs `82` (`+10`), coverage `84.59%` vs `85.50%` (`-0.91` pts). Because the fresh output now includes extra top-level metadata fields not present in truth (`analysisMode`, `timingMode`, `sourceStrategy`, `coverage`, `provenance`), the cleaner apples-to-apples view is the shared-path subset: on `236` common benchmarked paths, fresh passes `85` (`36.02%`) vs baseline `77` (`32.63%`), a `+8` pass / `+3.39` point improvement. Specific improvements cluster in early dialogue alignment (`dialogue_segments[0].end`, `[0].text`, `[1].start`, `[1].end`, `[1].text`, `[2].start`, `[2].end`, `[3].start`), plus lyric-speaker identity cleanup on `dialogue_segments[18-19].speaker(_id)` and better `speaker_profiles[1].inferred_traits` structure. Regressions are smaller but real: `dialogue_segments[3].confidence`, `[8].end`, `[12].end`, `[13].start`, and a few speaker-profile trait/descriptor fields. The biggest remaining mismatch buckets in the fresh run are speaker-profile linkage (`35`), segment timing (`33`), segment speaker identity (`30`), speaker-profile traits (`22`), speaker-profile descriptors (`20`), and segment text (`18`). The single most important transcript-level failure remains late-trailer completeness: the fresh run still drops the final truth segment block starting at `dialogue_segments[20]` through `[29]`. In short: better than the old baseline, especially early, but still far from truth because the late montage/lyrics/promo section and speaker-linkage grounding remain unstable.
