...
test('Phase3 report scripts compute their timeline directly from chunkAnalysis', async (t) => {
  t.beforeEach(() => {
  ...
  await t.test('metrics.cjs does not emit empty placeholder metrics', async () => {
    ...
    is(metricsData.implementationStatus.dataSource, 'derived-from-phase2.chunkAnalysis');
  ...
  await t.test('emotional-analysis.cjs does not emit zero-second/null placeholder analysis', async () => {
    ...
    is(saved.implementationStatus.dataSource, 'derived-from-phase2.chunkAnalysis');
  ...
  await t.test('failed placeholder chunks are excluded from aggregation', async () => {
