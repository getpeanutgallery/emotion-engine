...
test('Get Dialogue Script', async (t) => {
  ...
  t.test('run function', (tNested) => {
    ...
    tNested.test('retries invalid dialogue JSON before succeeding', async () => {
      ...
      ok(completionPrompts[0].includes('Transcribe the audible dialogue and dialogue-like vocal material in this audio.'));
      ok(!completionPrompts[0].includes('LOCAL TOOL LOOP:'));
      ok(completionPrompts[1].includes('LOCAL VALIDATION REPAIR:'));
      ok(completionPrompts[1].includes('same required schema'));
    });
    ...
    tNested.test('captures AI + ffmpeg raw artifacts when debug.captureRaw=true', async () => {
      ...
      ok(completionPrompts.some((prompt) => prompt.includes('Transcribe the audible dialogue and dialogue-like vocal material in this audio.')));
      ok(completionPrompts.every((prompt) => !prompt.includes('LOCAL TOOL LOOP:')));
    ...
    tNested.test('forces chunking for long within-budget dialogue to stabilize timing', async () => {
      ...
      ok(completionPrompts.some((prompt) => prompt.includes('Create a new speaker_id only when at least TWO stable acoustic signals disagree with the current voice profile.')));
      ok(completionPrompts.some((prompt) => prompt.includes('Speaker continuity is acoustic, not semantic')));
      ok(completionPrompts.some((prompt) => prompt.includes('Treat the handoff speaker registry as continuity memory and default to reusing a prior speaker_id unless there is clear acoustic contradiction.')));
      ok(completionPrompts.some((prompt) => prompt.includes('Minimize speaker-id proliferation across chunks: one-line speaker IDs are discouraged unless there is strong explicit acoustic contradiction.')));
      ...
      ok(completionPrompts.some((prompt) => prompt.includes('Include audibly supported spoken dialogue and dialogue-like vocal material, including short spoken fragments when only part of a weak, filtered, or partially masked line is recoverable.')));
      ok(completionPrompts.some((prompt) => prompt.includes('Do not drop or trim audibly supported spoken words merely because musical score is present underneath, the delivery has melodic contour, or the phrase might also resemble lyrics.')));
  ...
  t.test('output structure', (tNested) => {
    ...
    tNested.test('transcription prompt instructs grounded vs inferred speaker separation', async () => {
      ...
      ok(completionPrompts[0].includes('Keep inferred_traits clearly speculative and separate from grounded same-speaker evidence'));
      ok(completionPrompts[0].includes('inferred_traits must always be present as an object with a traits array'));
      ok(completionPrompts[0].includes('If you are unsure or have no supportable speculative traits, return exactly "inferred_traits": { "traits": [] }'));
      ok(completionPrompts[0].includes('Every inferred_traits.traits[*] entry must be an object with at least "trait" and "value" string fields'));
      ok(completionPrompts[0].includes('Valid inferred trait example: { "trait": "accent", "value": "possibly Midwestern US", "confidence": 0.31, "note": "speculative" }'));
      ok(completionPrompts[0].includes('Do not return bare strings, descriptor-only objects, or grounded evidence inside inferred_traits'));
      ...
      ok(completionPrompts[0].includes('For adjacent or near-adjacent lines, default to reusing the current speaker_id unless there is clear acoustic contradiction.'));
      ok(completionPrompts[0].includes('Minimize speaker-id proliferation: one-line speaker IDs are discouraged unless the line has strong explicit acoustic contradiction against nearby continuity.'));
      ok(completionPrompts[0].includes('Include audibly supported spoken dialogue, including short spoken fragments when only part of a weak, filtered, or partially masked line is recoverable.'));
      ok(completionPrompts[0].includes('Do not drop or trim audibly supported spoken words merely because musical score is present underneath, the delivery has melodic contour, or the phrase might also resemble lyrics.'));
      ...
      ok(completionPrompts[0].includes('If a spoken line is real but weak, filtered, reverberant, synthetic-sounding, or only partly recoverable, keep the audible portion as its own segment instead of dropping it.'));
      ok(completionPrompts[0].includes('Do not absorb a short weak spoken line into stronger neighboring lines merely because the neighboring lines are clearer, longer, or easier to summarize.'));
      ok(completionPrompts[0].includes('Low confidence is an allowed outcome. If spoken words are audibly supported but faint or uncertain, preserve them with reduced confidence rather than suppressing them.'));
      ...
      ok(completionPrompts[0].includes('When speech is partially masked, clipped, distant, distorted, reverberant, filtered, synthetic-sounding, or overlapped, prefer a short literal fragment of what is actually audible'));
