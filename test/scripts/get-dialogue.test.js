...
test('Get Dialogue Script', async (t) => {
  ...
  t.test('run function', (tNested) => {
    ...
    tNested.test('forces chunking for long within-budget dialogue to stabilize timing', async () => {
      ...
      ok(completionPrompts.some((prompt) => prompt.includes('Exclude sung lyrics, chant-like vocals, rap synchronized to music, melodic refrains, and other music-led vocal phrases from dialogue_segments')));
      ok(completionPrompts.some((prompt) => prompt.includes('If delivery changes mode between spoken dialogue and music-led vocals, split them into adjacent segments instead of merging them')));
      ...
      ok(completionPrompts.some((prompt) => prompt.includes('Do not compress the whole chunk\'s dialogue into the opening seconds or pull later spoken lines earlier to cover a larger region')));
      is(result.artifacts.dialogueData.totalDuration, 10);
  ...
  t.test('output structure', (tNested) => {
    ...
    tNested.test('transcription prompt instructs grounded vs inferred speaker separation', async () => {
      ...
      ok(completionPrompts[0].includes('Do not compress the whole file\'s dialogue into the opening seconds or pull later spoken lines earlier to cover a larger region'));
      ok(completionPrompts[0].includes('If no audible spoken words are detected, return an empty dialogue_segments array'));
