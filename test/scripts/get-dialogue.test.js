...
test('Get Dialogue Script', async (t) => {
  ...
  t.test('run function', (tNested) => {
    ...
    tNested.test('forces chunking for long within-budget dialogue to stabilize timing', async () => {
      ...
      ok(completionPrompts.some((prompt) => prompt.includes('Before reusing a prior speaker_id, compare voice quality, timbre, delivery mode, recording texture, apparent age range, gender presentation, and accent/dialect impression')));
