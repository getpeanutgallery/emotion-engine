...
} = require('../../server/lib/faster-whisper-music-vocals-timing.cjs');
...
test('deriveFasterWhisperMusicVocalsTiming spawns the repo-local bridge with repo-local caches', async (t) => {
  const workspaceDir = makeTempDir('ee-fw-music-helper-success-');
  ...
  const calls = [];
  const result = await deriveFasterWhisperMusicVocalsTiming({
    ...
    spawnImpl: (command, args, options) => {
      ...
      return createFakeChild({
        stdoutText: JSON.stringify({
          dialogue_segments: [
            { text: 'hello from the chorus', start: 1.1, end: 2.4, words: [{ word: 'hello', start: 1.1, end: 1.5 }] }
          ],
          summary: '',
          totalDuration: 9.5,
  ...
  assert.equal(result.musicVocalsData.vocal_segments[0].text, 'hello from the chorus');
...
test('deriveFasterWhisperMusicVocalsTiming fails when the bridge returns no timed segments', async (t) => {
  const workspaceDir = makeTempDir('ee-fw-music-helper-empty-');
  t.after(() => fs.rmSync(workspaceDir, { recursive: true, force: true }));
  ...
  await assert.rejects(() => deriveFasterWhisperMusicVocalsTiming({
    assetPath: '/tmp/fake-asset.mp4',
    pythonPath,
    scriptPath,
    spawnImpl: () => createFakeChild({
      stdoutText: JSON.stringify({
        dialogue_segments: [{ text: 'hello from the chorus' }],
        summary: '',
        totalDuration: 9.5,
  ...
  }), /did not return any timed music-vocals segments/);
});

test('deriveFasterWhisperMusicVocalsTiming surfaces bridge stderr on non-zero exit', async (t) => {
  const workspaceDir = makeTempDir('ee-fw-music-helper-error-');
  t.after(() => fs.rmSync(workspaceDir, { recursive: true, force: true }));
  ...
  await assert.rejects(async () => {
    await deriveFasterWhisperMusicVocalsTiming({
      assetPath: '/tmp/fake-asset.mp4',
      pythonPath,
      scriptPath,
      spawnImpl: () => createFakeChild({
    ...
    assert.equal(error.name, 'FasterWhisperMusicVocalsTimingError');
