...
const PERSONA = { 
    ...
    description: '17yo Gen Z, 200+ TikToks/day, scrolls instantly if bored',
...
async function analyzeChunkWithContext(chunkPath, index, total, startTime, endTime, context) {
    ...
    if (context.dialogueContext) {
        fullContext += `**What You're Hearing (Dialogue):**\n${context.dialogueContext}\n\n`;
    }
    
    if (context.musicContext) {
        fullContext += `**What You're Hearing (Music/Audio):**\n${context.musicContext}\n\n`;
    }
    
    const prompt = `You are ${PERSONA.name}, ${PERSONA.description}.

Analyzing video chunk ${index + 1}/${total} (${startTime}s-${endTime}s).
${fullContext}**What You See (Visuals):** Describe the visual content of this chunk.

**Your Emotional Reaction:**
- Patience: rate 1-10
- Boredom: rate 1-10  
- Excitement: rate 1-10

**Your Thoughts:** Internal monologue (authentic Gen Z voice, raw and honest)

**Scroll Risk:** Are you about to scroll away? (low/medium/high/SCROLLING)

**Summary:** 2-3 sentences for next chunk's context

Format as JSON: { visuals, patience, boredom, excitement, thought, scroll_risk, summary }`;

    console.log('   Sending to Qwen (with context)...');
